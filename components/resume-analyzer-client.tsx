...
const copy = {
  en: {
    ...
    authError: "Anthropic authentication failed. Please check ANTHROPIC_API_KEY.",
  ...
  zh: {
    ...
    apiPermission:
      "服务商拒绝了这次请求。请检查模型名、令牌分组、余额和 API 路由配置。",
    authError: "Anthropic 认证失败，请检查 ANTHROPIC_API_KEY。",
...
function getErrorMessage(payload: ApiError, labels: (typeof copy)[Locale]) {
  if (payload.code === "permission_error") {
    ...
    return detail
      ? `${labels.apiPermission} Provider message: ${detail}${requestId}`
      : `${labels.apiPermission}${requestId}`;
  }
    ...
    <main className="min-h-screen bg-teal-50 pt-16">
      <section className="mx-auto grid w-full max-w-[1220px] gap-6 px-5 py-8 sm:px-8 lg:grid-cols-[0.95fr_1.05fr] lg:px-12 lg:py-12">
        ...
        >
          ...
          {error ? (
            <div className="mt-5 rounded-xl border border-red/25 bg-red/10 p-4 text-sm font-semibold leading-6 text-red">
              {error}
            </div>
          ) : null}
