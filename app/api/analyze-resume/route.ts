...
function isMissingSdkError(error: unknown) {
...
function parseAnthropicErrorBody(body: string): ApiErrorPayload | null {
  try {
    ...
    if (error.status === 403) {
      return {
        body: {
          ...
          error:
            error.message ||
            "The provider rejected this request. Check model access, token group, balance, or region.",
    ...
    if (error.status === 401) {
      return {
        body: {
          ...
          error:
            "Anthropic authentication failed. Please check ANTHROPIC_API_KEY."
        },
        status: 401
      };
    }
...
async function createMessage(request: MessageRequest, apiKey: string) {
  if (process.env.ANTHROPIC_BASE_URL) {
    return createMessageWithFetch(request, apiKey);
  }
...
async function createMessageWithFetch(request: MessageRequest, apiKey: string) {
  ...
  if (!response.ok) {
    ...
    const parsed = parseAnthropicErrorBody(body);
  ...
  "summary": "<2-3 sentence overall assessment>"
    ...
    for (const model of getModelCandidates()) {
      try {
        ...
        if (error instanceof AnthropicApiError && error.status === 403) {
